...
def propose_alpha(alpha:torch.Tensor, sigma:torch.Tensor, smoothing:float):
    ...
    tau = ((alpha * smoothing).exp() - 1).log() / smoothing
    tau_prop = dist.Normal(
                        loc = tau,
                        scale = sigma
                    ).sample()
    alpha_prop = ((tau_prop * smoothing).exp() + 1).log() / smoothing
...
def acceptance_alpha(alpha:torch.Tensor, alpha_prop:torch.Tensor, 
    ...
    log_transform = (1 - (-smoothing * alpha_prop).exp()).log() - (1 - (-smoothing * alpha).exp()).log()
