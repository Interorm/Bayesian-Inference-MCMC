...
def propose_alpha(alpha:torch.Tensor, sigma:torch.Tensor, smoothing:float):

    tau = (((alpha + 1).clamp(1e-5) * smoothing).exp() - 1).log() / smoothing
    tau_prop = dist.Normal(
                        loc = tau,
                        scale = sigma
                    ).sample()
    alpha_prop = ((tau_prop * smoothing).exp() + 1).log() / smoothing - 1
...
def acceptance_alpha(alpha:torch.Tensor, alpha_prop:torch.Tensor, 
                     ...
                     alpha_prior:tuple, smoothing:float):
    ...
    log_transform = (1 - (-smoothing * (alpha_prop + 1 + 1e-5)).exp()).log() - (1 - (-smoothing * (alpha + 1 + 1e-5)).exp()).log()


    log_acceptance = (log_joined_gamma + log_indiv_gamma + log_kernel)[:, :, :-1] + log_prior + log_transform
...
def alpha_step(alpha:torch.Tensor, sigma:torch.Tensor, 
               beta:torch.Tensor, density:torch.Tensor, 
               alpha_prior:tuple, smoothing:float=1):

    alpha_prop = propose_alpha(alpha, sigma, smoothing)

    acceptance = acceptance_alpha(alpha, alpha_prop, beta, density, alpha_prior, smoothing)
